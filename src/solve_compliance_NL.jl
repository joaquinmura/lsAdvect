...
This version uses the NLopt interface
...
For this setting, the domain Ω is embeeded into a larger one D, where 
Ω = { x ∈ D : ϕ(x)<0 }
...
"""
function solve_compliance_NL(Ω::Triangulation,phi,E,ν,sop::ShapeOptParams)
        
#! STILL IS A COPY OF solve_compliance.JL
    ...
    volume     = []
    η_vol      = []    # list with Lagrange multipliers for volume constraint
    ...
    push!(volume,vol)      
    push!(h_vol,vol - vol_target*1.5) # vol_max extended for the very first time
    ...
    for k in 1:sop.max_iter
        ...
        #! η = secant_lag_ctrl(η_vol,vol,volume,k,Vmax=10,Nvol = 10)
        push!(η_vol,η)

        if accepted_step
            #η = sop.vol_penal .* opt_region

            ∇ϕ = upwind2d_step(Ω,xc,phi0,Vc_ .- (η.*opt_region), curvature_penalty = sop.curv_penal)
        end
